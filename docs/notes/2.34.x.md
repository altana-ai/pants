...
Thank you to [Klaviyo](https://www.klaviyo.com/) and Depot (https://depot.dev/) for their Platinum tier support through throughout this release.
...
Fixed S3 downloads failing with HTTP 403 `SignatureDoesNotMatch` when the object key contains a character that must be percent-encoded in a URL, such as `+`. The key is now percent-encoded before the request is signed, and SigV4 requests are signed with botocore's S3-specific signer, so the signature covers exactly the path S3 receives. Keys that are already percent-encoded in the requested URL are unaffected.
...
Fixed `[go-test].force_race` and the `go_package` fields `test_race`, `test_msan` and `test_asan` being silently ignored. The `test` goal built its Go build options without requesting the test-specific fields, so the sanitizer settings never reached the test binary and only the owning `go_mod`'s `race` field had any effect.
